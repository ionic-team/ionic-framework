...
it('should open correct accordions when accordion group value is set', async () => {
  ...
  expect(
    accordions[0].classList.contains(
      'accordion-collapsed'
    )
  ).toEqual(true);
  expect(
    accordions[1].classList.contains(
      'accordion-collapsed'
    )
  ).toEqual(false);
  expect(
    accordions[2].classList.contains(
      'accordion-collapsed'
    )
  ).toEqual(true);
});
...
it('should set default values if not provided', async () => {
   ...
   */
  expect(accordion.value).toContain(
    'ion-accordion-'
  );
