...
configs({
  ...
  test.describe(
    title(
      'datetime-button: multiple selection'
    ),
    () => {
      test('should render number of dates when more than 1 date is selected', async ({
        page,
      }) => {
      ...
      test('should render number of dates when 0 dates are selected', async ({
        page,
      }) => {
        await page.setContent(
          `
      ...
      test('should render date when only 1 day is selected', async ({
        page,
      }) => {
        await page.setContent(
          `
      ...
      `,
        ...
        await expect(
          page.locator('#date-button')
        ).toHaveText('Jun 1, 2022');
      });
      test('should use customFormatter', async ({
        page,
      }) => {
        await page.setContent(
          `
      ...
      test('should re-render when value is programmatically changed', async ({
        page,
      }) => {
        await page.setContent(
          `
      ...
      test('should render single date if datetime is used incorrectly', async ({
        page,
      }) => {
        await page.setContent(
          `
