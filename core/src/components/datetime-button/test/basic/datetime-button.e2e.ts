...
configs({
  ...
  test.describe(
    title(
      'datetime-button: switching to correct view'
    ),
    () => {
      test.beforeEach(
        async ({ page }) => {
      ...
      test('should switch to a date-only view when the date button is clicked', async ({
        page,
      }) => {
      ...
      test('should switch to a time-only view when the time button is clicked', async ({
        page,
      }) => {
        const datetime = page.locator(
          'ion-datetime'
  ...
  test.describe(
    title('datetime-button: labels'),
    () => {
      test('should set date and time labels in separate buttons', async ({
        page,
      }) => {
        await page.setContent(
          `
      ...
      `,
        ...
        await expect(
          page.locator('#date-button')
        ).toContainText('Jan 1, 2022');
        await expect(
          page.locator('#time-button')
        ).toContainText('6:30 AM');
      });
      test('should set only month and year', async ({
        page,
      }) => {
        await page.setContent(
          `
      ...
      `,
        ...
        await expect(
          page.locator('#date-button')
        ).toContainText('January 2022');
        await expect(
          page.locator('#time-button')
        ).toBeHidden();
      });
      test('should set only year', async ({
        page,
      }) => {
        await page.setContent(
          `
      ...
      test('should set only month', async ({
        page,
      }) => {
        await page.setContent(
          `
      ...
      test('should set only time', async ({
        page,
      }) => {
        await page.setContent(
          `
      ...
      test('should update the label when the value of the datetime changes', async ({
        page,
      }) => {
        await page.setContent(
          `
      ...
      `,
        ...
        await expect(
          dateTarget
        ).toContainText('May 10, 2023');
      });
      test('should set only month and year when only passing month and year', async ({
        page,
      }, testInfo) => {
        testInfo.annotations.push({
          type: 'issue',
          description:
            'https://github.com/ionic-team/ionic-framework/issues/27797',
        });
      ...
      test('should set only year when passing only year', async ({
        page,
      }, testInfo) => {
        testInfo.annotations.push({
          type: 'issue',
  ...
  test.describe(
    title('datetime-button: locale'),
    () => {
      test('should use the same locale as datetime', async ({
        page,
      }) => {
        await page.setContent(
          `
      ...
      test('should respect hour cycle even if different from locale default', async ({
        page,
      }) => {
        await page.setContent(
          `
      ...
      test('should ignore the timezone when selecting a date', async ({
        page,
      }) => {
        await page.setContent(
          `
      ...
      `,
        ...
        const firstOfMonth =
          page.locator(
            'ion-datetime .calendar-day[data-month="1"][data-day="1"]'
          );
  ...
  test.describe(
    title(
      'datetime-button: formatOptions'
    ),
    () => {
      test('should include date and time for presentation date-time', async ({
        page,
      }) => {
        await page.setContent(
          `
      ...
      `,
        ...
        await expect(
          page.locator('#time-button')
        ).toContainText('01:22 AM');
      });

      test('should include date for presentation date', async ({
        page,
      }) => {
        await page.setContent(
          `
      ...
      test('should include date and time in same button for preferWheel', async ({
        page,
      }) => {
        await page.setContent(
          `
