...
configs().forEach(({ title, config }) => {
  test.describe('datetime-button: rendering', () => {
    test(title('should size the modal correctly'), async ({ page }) => {
    ...
    test(title('should size the popover correctly'), async ({ page }) => {
      await page.setContent(
        `
        <ion-datetime-button datetime="datetime"></ion-datetime-button>
        <ion-popover>
...
configs({ modes: ['md'], directions: ['ltr'] }).forEach(({ title, config }) => {
  test.describe('datetime-button: popover', () => {
    ...
    test(title('should open the date popover'), async ({ page }) => {
    ...
    test(title('should open the time popover'), async ({ page }) => {
    ...
    test(title('should open the date popover then the time popover'), async ({ page }) => {
      await page.locator('#date-button').click();
      await ionPopoverDidPresent.next();
      expect(datetime).toBeVisible();
    ...
    test(title('should open the time popover then the date popover'), async ({ page }) => {
      await page.locator('#time-button').click();
      await ionPopoverDidPresent.next();
      expect(datetime).toBeVisible();
...
configs({ modes: ['md'], directions: ['ltr'] }).forEach(({ title, config }) => {
  test.describe('datetime-button: modal', () => {
    ...
    test(title('should open the date modal'), async ({ page }) => {
      await page.locator('#date-button').click();
    ...
    test(title('should open the time modal'), async ({ page }) => {
      await page.locator('#time-button').click();
    ...
    test(title('should open the date modal then the time modal'), async ({ page }) => {
      await page.locator('#date-button').click();
      await ionModalDidPresent.next();
      expect(datetime).toBeVisible();
    ...
    test(title('should open the time modal then the date modal'), async ({ page }) => {
      await page.locator('#time-button').click();
      await ionModalDidPresent.next();
      expect(datetime).toBeVisible();
