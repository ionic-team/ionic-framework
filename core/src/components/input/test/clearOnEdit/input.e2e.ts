...
test.describe('input: clearOnEdit', () => {
  test('should clear the input on first keystroke of input being focused', async ({ page }) => {
    await page.setContent(`<ion-input label="my input" value="some value" clear-on-edit="true"></ion-input>`);
  ...
  test('should not clear the input if it does not have an initial value when typing', async ({ page }) => {
    await page.setContent(`<ion-input label="my input" value="" clear-on-edit="true"></ion-input>`);
  ...
  test('should emit ionChange once on blur', async ({ page }, testInfo) => {
    ...
    await page.setContent(`<ion-input label="my input" placeholder="input" clear-on-edit="true"></ion-input>`);
