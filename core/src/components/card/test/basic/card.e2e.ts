...
configs().forEach(
  ({ title, screenshot, config }) => {
    test.describe(
      ...
      () => {
        test('should not have visual regressions with basic card', async ({
          page,
        }) => {
...
configs({
  ...
  ({ title, screenshot, config }) => {
    test.describe(
      title('card: feature rendering'),
      () => {
        test('should not have visual regressions with button cards', async ({
          page,
        }) => {
          await page.setContent(
            `
      ...
      `,
        ...
        test('should not have visual regressions with disabled card', async ({
          page,
        }) => {
          await page.setContent(
            `
      ...
      `,
          ...
          await expect(
            ...
            screenshot(`card-color`)
          );
        });
        test('headings should have correct size in card', async ({
          page,
        }) => {
          await page.setContent(
            `
      ...
      `,
          ...
          await expect(
            ...
            screenshot(`card-headings`)
          );
        });
        test('should render even without header or content elements', async ({
          page,
        }) => {
          await page.setContent(
            `
...
configs({
  ...
  ({ title, screenshot, config }) => {
    test.describe(
      title('card: translucent'),
      () => {
        test('should not have visual regressions with translucent cards', async ({
          page,
        }) => {
          await page.setContent(
            `
