...
configs().forEach(({ title, config }) => {
  test.describe('radio: rendering', () => {
    test(title('should correctly render an unchecked radio group'), async ({ page }) => {
    ...
    test(title('should correctly render a checked radio group'), async ({ page }) => {
      await page.setContent(
        `
    ...
    test(title('should allow shadow parts to be styled'), async ({ page }) => {
      await page.setContent(
        `
        <ion-radio-group value="sausage">
          <ion-item>
    ...
    test(title('should apply color correctly'), async ({ page }) => {
      await page.setContent(
        `
...
configs({ directions: ['ltr'] }).forEach(({ title, config }) => {
  test.describe('radio: interaction', () => {
    test(title('radio should be checked when activated'), async ({ page }) => {
      await page.setContent(
        `
        <ion-radio-group>
          <ion-item>
    ...
    test(title('radio should be checked when activated even without a radio group'), async ({ page }) => {
      await page.setContent(
        `
        <ion-radio slot="start" value="pepperoni"></ion-radio>
      `,
