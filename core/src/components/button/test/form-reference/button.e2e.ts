...
configs().forEach(({ title, config }) => {
  test.describe('button: form', () => {
    test(title('should submit the form by id'), async ({ page }) => {
    ...
    test(title('should submit the form by reference'), async ({ page }) => {
      await page.setContent(
        `
        <form></form>
    ...
    test(title('should submit the closest form'), async ({ page }) => {
      await page.setContent(
        `
        <form>
           <ion-button type="submit">Submit</ion-button>
    ...
    test.describe('should throw a warning if the form cannot be found', () => {
      test(title('form is a string selector'), async ({ page }) => {
      ...
      test(title('form is an element reference'), async ({ page }) => {
        await page.setContent(
          `
          <ion-button type="submit">Submit</ion-button>
          <script>
