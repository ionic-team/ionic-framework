...
export class Nav implements NavOutlet {
  ...
  popTo(
    ...
    if (
      typeof indexOrViewCtrl ===
        'object' &&
      (
        indexOrViewCtrl as ViewController
      ).component
    ) {
  ...
  private canGoBackSync(
    ...
    return !!(
      view && this.getPreviousSync(view)
    );
  }
  ...
  private async queueTrns(
    ...
    if (
      ...
      if (router) {
        ...
        if (
          typeof canTransition ===
          'string'
        ) {
          router.push(
            canTransition,
            ti.opts!.direction || 'back'
          );
          return false;
        }
      }
  ...
  private prepareTI(
    ...
    if (ti.removeView !== undefined) {
      ...
      const index = this.views.indexOf(
        ti.removeView
      );
      if (index < 0) {
        throw new Error(
  ...
  private transitionFinish(
     ...
     */
    const activeView = hasCompleted
      ? enteringView
      : leavingView;
    if (activeView) {
      this.unmountInactiveViews(
        activeView
      );
    }
