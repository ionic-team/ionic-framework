...
const setToolbarBorderChange = (toolbar: ToolbarIndex, style: string | undefined) => {
  if (style === undefined) {
    toolbar.background.style.removeProperty('--border-style');
  } else {
    toolbar.background.style.setProperty('--border-style', style.toString());
  }
};
...
const handleToolbarBorderIntersection = (ev: any, mainHeaderIndex: HeaderIndex) => {
  ...
  setToolbarBorderChange(mainHeaderIndex.toolbars[0], (scale === 1) ? undefined : 'none');
};
  ...
  writeTask(() => {
    ...
    if (event.isIntersecting) {
      ...
      if (hasValidIntersection) {
        ...
        setToolbarBorderChange(mainHeaderIndex.toolbars[0], undefined);
      }
    }
  });
};
