...
configs().forEach(
  ({ title, screenshot, config }) => {
    test.describe(
      ...
      () => {
        test('should not have visual regressions with basic header', async ({
          page,
        }) => {
...
configs({
  ...
  ({ title, screenshot, config }) => {
    test.describe(
      ...
      () => {
        test('should not have visual regressions with no border', async ({
          page,
        }) => {
          await page.setContent(
            `
...
configs({
  ...
  ({ title, screenshot, config }) => {
    test.describe(
      title('header: translucent'),
      () => {
        test('should not have visual regressions with translucent header', async ({
          page,
        }) => {
          await page.setContent(
            `
      ...
      `,
          ...
          await expect(
            ...
            screenshot(
              `header-translucent-diff`
            )
          );
        });

        test('should not have visual regressions with translucent header with color', async ({
          page,
        }) => {
          await page.setContent(
            `
...
configs({
  ...
  ({ title, screenshot, config }) => {
    test.describe(
      title('header: translucent'),
      () => {
        test('should not hide MD headers when using a descendant iOS header in an MD app', async ({
          page,
        }) => {
          test.info().annotations.push({
            type: 'issue',
            description:
              'https://github.com/ionic-team/ionic-framework/issues/28867',
          });
          await page.setContent(
            `
      ...
      `,
          ...
          await expect(
            ...
            screenshot(
              'header-md-visibility-ios-descendant'
            )
          );
        });
        test('should not hide MD headers when using a root iOS header in an MD app', async ({
          page,
        }) => {
          test.info().annotations.push({
            type: 'issue',
