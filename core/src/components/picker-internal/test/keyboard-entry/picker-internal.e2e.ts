...
configs({
  ...
  test.describe(
    title(
      'picker-internal: keyboard entry'
    ),
    () => {
      test('should scroll to and update the value prop for a single column', async ({
        page,
      }) => {
      ...
      `,
        ...
        await expect(
          column
        ).toHaveJSProperty('value', 2);
      });

      test('should scroll to and update the value prop for multiple columns', async ({
        page,
      }) => {
        await page.setContent(
          `
      ...
      `,
        ...
        const highlight = page.locator(
          'ion-picker-internal .picker-highlight'
        );
        const firstIonChange = await (
          firstColumn as E2ELocator
        ).spyOnEvent('ionChange');
        const secondIonChange = await (
          secondColumn as E2ELocator
        ).spyOnEvent('ionChange');

        const box =
          await highlight.boundingBox();
        if (box !== null) {
        ...
        await expect(
          secondColumn
        ).toHaveJSProperty('value', 24);
      });

      test('should select 00', async ({
        page,
      }) => {
        await page.setContent(
          `
      ...
      `,
        ...
        await expect(
          column
        ).toHaveJSProperty('value', 12);
      });
      test('pressing Enter should dismiss the keyboard', async ({
        page,
      }) => {
        test.info().annotations.push({
          type: 'issue',
          description:
            'https://github.com/ionic-team/ionic-framework/issues/28325',
        });
        await page.setContent(
          `
        <ion-picker-internal>
