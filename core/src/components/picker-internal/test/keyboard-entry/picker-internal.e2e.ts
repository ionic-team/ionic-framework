...
configs().forEach(({ title, config }) => {
  test.describe('picker-internal: keyboard entry', () => {
    test(title('should scroll to and update the value prop for a single column'), async ({ page }) => {
    ...
    test(title('should scroll to and update the value prop for multiple columns'), async ({ page }) => {
      await page.setContent(
        `
        <ion-picker-internal>
