...
configs({
  ...
  ({ title, screenshot, config }) => {
    test.describe(
      title(
        'picker-internal: rendering'
      ),
      () => {
        test('inline pickers should not have visual regression', async ({
          page,
        }) => {
          await page.goto(
            `/src/components/picker-internal/test/basic`,
            config
          );

          const fullStack =
            page.locator(
              '#inline button[data-value="full-stack"]'
            );
          const onion = page.locator(
            '#inline button[data-value="onion"]'
          );
    ...
    test.describe(
      title(
        'picker-internal: overlay rendering'
      ),
      () => {
        test('popover: should not have visual regression', async ({
          page,
        }) => {
          await page.goto(
            `/src/components/picker-internal/test/basic`,
          ...
          const pickerInternal =
            page.locator(
              'ion-popover ion-picker-internal'
            );
        ...
        test('modal: should not have visual regression', async ({
          page,
        }) => {
          await page.goto(
            '/src/components/picker-internal/test/basic',
...
configs({
  ...
  test.describe(
    ...
    () => {
      ...
      test('tabbing should correctly move focus between columns', async ({
        page,
      }) => {
        const firstColumn =
          page.locator(
            'ion-picker-column-internal#first'
          );
        const secondColumn =
          page.locator(
            'ion-picker-column-internal#second'
          );
      ...
      test('tabbing should correctly move focus back', async ({
        page,
      }) => {
        const firstColumn =
          page.locator(
