...
configs().forEach(({ title, config }) => {
  test.describe('picker-internal', () => {
    test(title('inline pickers should not have visual regression'), async ({ page }) => {
    ...
    test.describe('picker-internal: focus', () => {
      ...
      test(title('tabbing should correctly move focus between columns'), async ({ page }) => {
      ...
      test(title('tabbing should correctly move focus back'), async ({ page }) => {
        const firstColumn = page.locator('ion-picker-column-internal#first');
        const secondColumn = page.locator('ion-picker-column-internal#second');
    ...
    test.describe('within overlay:', () => {
      ...
      test(title('popover: should not have visual regression'), async ({ page }) => {
        await page.goto(`/src/components/picker-internal/test/basic`, config);
      ...
      test(title('modal: should not have visual regression'), async ({ page }) => {
        await page.goto(`/src/components/picker-internal/test/basic`, config);
