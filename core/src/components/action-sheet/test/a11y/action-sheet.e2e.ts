...
const testAria = async (
  ...
  const ariaLabelledBy =
    await actionSheet.getAttribute(
      'aria-labelledby'
    );
...
configs({
  ...
  test.describe(
    title('action-sheet: a11y'),
    () => {
      test.beforeEach(
        async ({ page }) => {
          await page.goto(
            `/src/components/action-sheet/test/a11y`,
            config
          );
        }
      );
      test('should not have accessibility violations when header is defined', async ({
        page,
      }) => {
        const button = page.locator(
          '#bothHeaders'
      ...
      test('should have aria-labelledby when header is set', async ({
        ...
        await testAria(
          page,
          'bothHeaders',
          'action-sheet-1-header'
        );
      });

      test('should not have aria-labelledby when header is not set', async ({
        page,
      }) => {
        await testAria(
          page,
