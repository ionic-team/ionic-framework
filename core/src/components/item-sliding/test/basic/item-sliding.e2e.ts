...
test.describe('item-sliding: basic', () => {
  configs({ modes: ['md'] }).forEach(({ title, config }) => {
    test.describe('item-sliding: rendering', () => {
  ...
  configs({ modes: ['md'], directions: ['ltr'] }).forEach(({ title, config }) => {
    test.describe('item-sliding: functionality', () => {
      // mouse gesture is flaky on CI, skip for now
      test.fixme(title('should open when swiped'), async ({ page, skip }) => {
        skip.browser(
      ...
      test(title('should not scroll when the item-sliding is swiped'), async ({ page, skip }) => {
