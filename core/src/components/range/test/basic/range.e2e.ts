...
test.describe('range: basic', () => {
  ...
  configs({ directions: ['ltr'] }).forEach(({ title, config }) => {
    test.describe('range: functionality', () => {
      /**
       * The mouse events are flaky on CI
       */
      test.fixme(title('should emit start/end events'), async ({ page }, testInfo) => {
      ...
      test(title('should emit start/end events, keyboard'), async ({ page }) => {
      ...
      test(title('should not scroll when the knob is swiped'), async ({ page, skip }) => {
