    ...
    () => {
      test.describe(
        ...
        `,
          ...
          test('should emit end event on tap', async ({
            page,
          }, testInfo) => {
            testInfo.annotations.push({
              type: 'issue',
              description:
                'https://github.com/ionic-team/ionic-framework/issues/28487',
            });
             ...
             */
            const x =
              rangeBoundingBox!.width -
              50;
            // The y coordinate is the middle of the range.
            const y =
              rangeBoundingBox!.height /
              2;
      ...
      test.describe('ionChange', () => {
        ...
        // TODO FW-2873
        test.skip('should emit when the knob is released', async ({
          page,
        }) => {
          await page.setContent(
            `<ion-range aria-label="range"></ion-range>`,
        ...
        test('should emit when the knob is moved with the keyboard', async ({
          page,
        }) => {
          await page.setContent(
            `<ion-range aria-label="range" value="50"></ion-range>`,
            config
          );
      ...
      test.describe('ionInput', () => {
        ...
        test.skip('should emit when the knob is dragged', async ({
          page,
        }) => {
          await page.setContent(
            `<ion-range aria-label="range"></ion-range>`,
