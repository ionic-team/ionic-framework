  ...
  test.describe('popover: rendering', async () => {
    test(title('should not have visual regressions'), async ({ page }) => {
      ...
      for (const id of buttonIDs) {
        await screenshotPopover(page, config, id, 'basic');
      }
    });
  });
});
configs().forEach(({ title, config }) => {
  test.describe('popover: htmlAttributes', async () => {
    test(title('should inherit attributes on host'), async ({ page }) => {
...
configs().forEach(({ title, config }) => {
  test.describe('popover: focus trap', async () => {
    ...
    test(title('should focus the first ion-item on ArrowDown'), async ({ page }) => {
      await openPopover(page, 'basic-popover');
    ...
    test(title('should not override keyboard interactions for textarea elements'), async ({ page, browserName }) => {
