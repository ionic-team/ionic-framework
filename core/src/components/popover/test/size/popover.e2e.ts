    ...
    test.describe(
      ...
      async () => {
        test.beforeEach(
          async ({ page }) => {
            await page.goto(
              '/src/components/popover/test/size',
              config
            );
          }
        );
        ...
        test.describe(
          ...
          async () => {
             ...
             */
            test('should calculate popover width based on sizing method', async ({
              page,
            }) => {
              await page.setViewportSize(
                Viewports.tablet
                  .portrait
              ); // avoid popovers overlapping

              const sameTimeTriggers = [
                'auto-trigger',
                'cover-trigger',
                'event-trigger',
              ];
