...
configs({ modes: ['md'], directions: ['ltr'] }).forEach(({ title, config }) => {
  test.describe('popover: trigger', async () => {
    ...
    test(title('should open popover by left clicking on trigger'), async ({ page }) => {
      await openPopover(page, 'left-click-trigger');
      await checkPopover(page, '.left-click-popover');
    });

    test(title('should open popover by right clicking on trigger'), async ({ page }) => {
    ...
    test(title('should open popover by hovering over trigger'), async ({ page }) => {
      const ionPopoverDidPresent = await page.spyOnEvent('ionPopoverDidPresent');
    ...
    test(title('should still open popover when it has been removed and re-added to DOM'), async ({ page }) => {
