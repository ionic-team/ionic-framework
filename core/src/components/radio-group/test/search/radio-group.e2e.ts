...
configs({
  ...
  test.describe(
    ...
    () => {
      test('radio should remain checked after being removed/readded to the dom', async ({
        page,
      }) => {
        await page.goto(
          '/src/components/radio-group/test/search',
          config
        );

        const radioFixture =
          new RadioFixture(page);
