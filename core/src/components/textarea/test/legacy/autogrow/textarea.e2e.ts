    ...
    test.describe(
      ...
      () => {
        test('should not have visual regressions', async ({
          ...
          await page.goto(
            `/src/components/textarea/test/legacy/autogrow`,
            config
          );
          ...
          await expect(
            ...
            screenshot(
              `textarea-autogrow-diff`
            )
          );
        });

        test('should grow when typing', async ({
          page,
        }) => {
        ...
        test('should break long lines without white space', async ({
          page,
        }) => {
          test.info().annotations.push({
            type: 'issue',
            description:
              'https://github.com/ionic-team/ionic-framework/issues/25893',
          });
