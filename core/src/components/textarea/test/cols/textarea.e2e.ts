...
configs({
  ...
  ({ title, screenshot, config }) => {
    test.describe(
      title('textarea: cols'),
      () => {
        test('should respect cols when autogrow is not set', async ({
          page,
        }) => {
      ...
      `,
          ...
          await expect(
            ...
            screenshot('textarea-cols')
          );
        });
        test('should ignore cols when autogrow is set', async ({
          page,
        }) => {
          await page.setContent(
            `
