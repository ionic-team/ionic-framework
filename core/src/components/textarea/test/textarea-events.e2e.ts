...
configs({
        ...
        () => {
          test('should emit if the value has changed', async ({
            ...
            await page.setContent(
              `<ion-textarea aria-label="textarea"></ion-textarea>`,
              config
            );
          ...
          test('should emit if the textarea is cleared with an initial value', async ({
            page,
          }) => {
            await page.setContent(
  ...
  test.describe(
    title('textarea: events: ionInput'),
    () => {
      test('should emit when the user types', async ({
        page,
      }) => {
        await page.setContent(
          `<ion-textarea aria-label="textarea" value="some value"></ion-textarea>`,
          config
        );
        ...
        expect(
          ...
          value: 'new valuesome value',
          event: { isTrusted: true },
        });
      });

      test('should emit when the textarea is cleared on edit', async ({
        page,
      }) => {
        await page.setContent(
          `<ion-textarea clear-on-edit="true" value="some value"></ion-textarea>`,
          config
        );
