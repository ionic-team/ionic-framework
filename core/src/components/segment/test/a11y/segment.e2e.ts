...
configs().forEach(({ title, config }) => {
  test.describe('segment: a11y', () => {
    test(title('should not have any axe violations'), async ({ page }) => {
    ...
    test(title('segment buttons should be keyboard navigable'), async ({ page, browserName }) => {
      const tabKey = browserName === 'webkit' ? 'Alt+Tab' : 'Tab';
      const isRTL = config._direction === 'rtl';
