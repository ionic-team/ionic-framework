...
it('should correctly provide the collapsed breadcrumbs in the event payload', async () => {
  ...
  const onCollapsedClick = jest.fn(
    (ev) => ev
  );
  const breadcrumbs =
  ...
  const collapsedBreadcrumbs =
    onCollapsedClick.mock.calls[0][0]
      .detail.collapsedBreadcrumbs;
  expect(
    collapsedBreadcrumbs.length
  ).toEqual(3);
  expect(collapsedBreadcrumbs[0]).toBe(
    breadcrumb[1]
  );
  expect(collapsedBreadcrumbs[1]).toBe(
    breadcrumb[2]
  );
  expect(collapsedBreadcrumbs[2]).toBe(
    breadcrumb[3]
  );
});
...
it('should exclude the separator from narrators', async () => {
  ...
  const firstBreadcrumb =
    page.body.querySelector(
      'ion-breadcrumb:first-of-type'
    )!;
  const separator =
    firstBreadcrumb.shadowRoot!.querySelector(
      '[part="separator"]'
    )!;

  expect(
    separator.getAttribute(
      'aria-hidden'
    )
  ).toBe('true');
});
...
it('should have color attribute', async () => {
  ...
  expect(
    breadcrumbs.hasAttribute('color')
  ).toBe(true);
});
