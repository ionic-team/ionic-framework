  ...
  implements ComponentInterface
{
   ...
   */
  @Prop() cancelButtonIcon = config.get(
    'backButtonIcon',
    arrowBackSharp
  ) as string;
   ...
   */
  private onCancelSearchbar = async (
    ev?: Event
  ) => {
    if (ev) {
      ev.preventDefault();
  ...
  private positionCancelButton() {
    ...
    if (
      ...
      if (shouldShowCancel) {
        ...
        const offset =
          cancelButton.offsetWidth;
        if (offset > 0) {
          if (rtl) {
            cancelStyle.marginLeft =
              -offset + 'px';
          } else {
            cancelStyle.marginRight =
              -offset + 'px';
          }
        }
      }
