...
| `showCancelButton` | `show-cancel-button` | Sets the behavior for the cancel button. `true`: Shows the cancel button on focus. `false`: Disables the cancel button. `persist`: Shows the cancel button on focus but does not hide it until the user clicks it or calls the `hideSearch` method.                                                                  | `boolean \| string`                                                         | `false`           |
