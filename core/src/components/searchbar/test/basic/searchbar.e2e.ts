...
configs({
  ...
  test.describe(
    ...
    () => {
      ...
      test('should show cancel button on focus if show-cancel-button=focus', async ({
        page,
      }) => {
        const searchbar =
          page.locator('#basic');
      ...
      test('should not show cancel button on focus if show-cancel-button=never', async ({
        page,
      }) => {
  ...
  ({ title, screenshot, config }) => {
    test.describe(
      ...
      `,
        ...
        test('should render cancel and clear buttons', async ({
          page,
        }) => {
          await page.setContent(
            `
...
configs({
  ...
  ({ title, screenshot, config }) => {
    test.describe(
      title(
        'searchbar: feature rendering'
      ),
      () => {
        test('should render searchbar with color', async ({
          page,
        }) => {
          await page.setContent(
            `
      ...
      `,
        ...
        test('should render disabled searchbar', async ({
          page,
        }) => {
          await page.setContent(
            `
      ...
      `,
        ...
        test('should render custom search icon', async ({
          page,
        }) => {
          await page.setContent(
            `
      ...
      `,
          ...
          const icon = page.locator(
            'ion-searchbar ion-icon.searchbar-search-icon'
          );
...
configs({
  ...
  ({ title, screenshot, config }) => {
    test.describe(
      title(
        'searchbar: ios clear button text cut off'
      ),
      () => {
        test('text should not be cut off when clear button is hidden', async ({
          page,
        }) => {
          await page.setContent(
            `
      ...
      `,
          ...
          await expect(
            ...
            screenshot(
              `searchbar-text-clear-hidden`
            )
          );
        });

        test('text should be cut off when clear button is visible', async ({
          page,
        }) => {
          await page.setContent(
            `
...
configs({
  ...
  ({ title, screenshot, config }) => {
    test.describe(
      title(
        'searchbar: cancel button alignment'
      ),
      () => {
        test('should align with the back button when used in a toolbar', async ({
          page,
        }, testInfo) => {
          testInfo.annotations.push({
            type: 'issue',
            description:
              'https://github.com/ionic-team/ionic-framework/issues/28468',
          });
          await page.setContent(
            `
        <ion-header>
