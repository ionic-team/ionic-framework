...
describe('searchbar: rendering', () => {
  it('should inherit properties on load', async () => {
    ...
    expect(
      nativeEl.getAttribute('name')
    ).toBe('search');
    expect(
      nativeEl.getAttribute('maxlength')
    ).toBe('4');
    expect(
      nativeEl.getAttribute('minlength')
    ).toBe('2');
    expect(
  ...
  it('should inherit watched attributes', async () => {
    ...
    const nativeEl =
      searchbarEl.querySelector(
        'input'
      )!;

    expect(
      nativeEl.getAttribute('lang')
    ).toBe('en-US');
    expect(
      nativeEl.getAttribute('dir')
    ).toBe('ltr');

    searchbarEl.setAttribute(
      'lang',
      'es-ES'
    );
    searchbarEl.setAttribute(
