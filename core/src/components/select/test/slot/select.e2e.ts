...
configs().forEach(
  ({ title, screenshot, config }) => {
    test.describe(
      title(
        'select: start and end slots (visual checks)'
      ),
      () => {
...
configs({
  ...
  test.describe(
    title(
      'select: start and end slots (functionality checks)'
    ),
    () => {
      ...
      test('should not open select when slotted buttons are clicked', async ({
        page,
      }) => {
        await page.setContent(
          `
        ...
        await page.click(
          'ion-select ion-button[slot="end"]'
        );
