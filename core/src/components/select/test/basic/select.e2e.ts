  ...
  test.describe(
    ...
    () => {
      test.beforeEach(
        async ({ page }) => {
          await page.goto(
            '/src/components/select/test/basic',
            config
          );
        }
      );
      ...
      test.describe(
        'select: alert',
        () => {
          test('it should open an alert select', async ({
            page,
          }) => {
      ...
      test.describe(
        'select: action sheet',
        () => {
          test('it should open an action sheet select', async ({
            page,
          }) => {
      ...
      test.describe(
        'select: popover',
        () => {
          test('it should open a popover select', async ({
            page,
            skip,
          }) => {
            // TODO (FW-2979)
            ...
            const popoverOption1 =
              popover.locator(
                '.select-interface-option:first-of-type ion-radio'
              );
            await expect(
              popoverOption1
            ).toBeFocused();
...
configs({
  ...
  test.describe(
    title('select: multiple selects'),
    () => {
      test('should not open multiple alert windows when clicked multiple times', async ({
        page,
      }) => {
        test.info().annotations.push({
          type: 'issue',
          description:
            'https://github.com/ionic-team/ionic-framework/issues/25126',
        });
...
configs({
  ...
  test.describe(
    title('select: ionChange'),
    () => {
      test('should fire ionChange when confirming a value from an alert', async ({
        page,
      }) => {
      ...
      test('should fire ionChange when confirming a value from a popover', async ({
        page,
      }) => {
        await page.setContent(
          `
      ...
      test('should fire ionChange when confirming multiple values from a popover', async ({
        page,
      }) => {
        await page.setContent(
          `
      ...
      test('should fire ionChange when confirming a value from an action sheet', async ({
        page,
      }) => {
        await page.setContent(
          `
      ...
      test('clicking padded space within item should click the select', async ({
        page,
      }) => {
        await page.setContent(
          `
