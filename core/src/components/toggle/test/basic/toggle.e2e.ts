...
configs().forEach(({ title, config }) => {
  test.describe('toggle: basic', () => {
    test.beforeEach(async ({ page }) => {
      await page.goto(`/src/components/toggle/test/basic`, config);
    });
    ...
    test(title('should have proper class and aria role when checked'), async ({ page }) => {
    ...
    test(title('should fire change event with detail'), async ({ page }) => {
      const toggle = page.locator('#orange');
    ...
    test(title('should fire change event if checked prop is changed directly'), async ({ page }) => {
      const toggle = page.locator('#orange');
      const ionChange = await page.spyOnEvent('ionChange');
    ...
    test(title('should pass properties down to hidden input'), async ({ page }) => {
