...
configs({ modes: ['md'], directions: ['ltr'] }).forEach(({ title, config }) => {
  test.describe('modal: trigger', () => {
    ...
    test(title('should open modal by left clicking on trigger'), async ({ page }) => {
    ...
    test(title('should still open modal when it has been removed and re-added to DOM'), async ({ page }) => {
