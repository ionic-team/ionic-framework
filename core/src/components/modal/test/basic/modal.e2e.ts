...
configs().forEach(({ title, config }) => {
  test.describe('modal: focus trapping', () => {
    ...
    test(title('focus should be trapped inside of modal'), async ({ page, browserName }) => {
      /**
...
configs().forEach(({ title, config }) => {
  test.describe('modal: htmlAttributes inheritance', () => {
    test(title('should correctly inherit attributes on host'), async ({ page }) => {
      await page.goto('/src/components/modal/test/basic', config);
      const ionModalDidPresent = await page.spyOnEvent('ionModalDidPresent');
...
configs().forEach(({ title, config }) => {
  test.describe('modal: backdrop', () => {
    ...
    test(title('it should dismiss the modal when clicking the backdrop'), async ({ page }) => {
      await page.setViewportSize(Viewports.tablet.portrait);
...
configs().forEach(({ title, config }) => {
  test.describe('modal: incorrect usage', () => {
    ...
    test(title('it should warn when setting a breakpoint on a non-sheet modal'), async ({ page }) => {
      const ionModalDidPresent = await page.spyOnEvent('ionModalDidPresent');
    ...
    test(title('it should return undefined when getting the breakpoint on a non-sheet modal'), async ({ page }) => {
      const ionModalDidPresent = await page.spyOnEvent('ionModalDidPresent');
