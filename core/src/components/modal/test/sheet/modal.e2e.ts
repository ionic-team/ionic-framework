...
configs().forEach(({ title, config }) => {
  test.describe('sheet modal: backdrop', () => {
    ...
    test(title('should dismiss the sheet modal when clicking the active backdrop'), async ({ page }) => {
      const ionModalDidPresent = await page.spyOnEvent('ionModalDidPresent');
    ...
    test(title('should present another sheet modal when clicking an inactive backdrop'), async ({ page }) => {
      const ionModalDidPresent = await page.spyOnEvent('ionModalDidPresent');
    ...
    test(title('input outside sheet modal should be focusable when backdrop is inactive'), async ({ page }) => {
      const ionModalDidPresent = await page.spyOnEvent('ionModalDidPresent');
...
configs().forEach(({ title, config }) => {
  test.describe('sheet modal: setting the breakpoint', () => {
    ...
    test.describe('sheet modal: invalid values', () => {
      ...
      test(title('it should not change the breakpoint when setting to an invalid value'), async ({ page }) => {
        const modal = await page.locator('ion-modal');
        const breakpoint = await modal.evaluate((el: HTMLIonModalElement) => el.getCurrentBreakpoint());
        expect(breakpoint).toBe(0.25);
      });
      test(title('it should warn when setting an invalid breakpoint'), async () => {
    ...
    test.describe('sheet modal: valid values', () => {
      ...
      test(title('should update the current breakpoint'), async ({ page }) => {
      ...
      test(title('should emit ionBreakpointDidChange'), async ({ page }) => {
        const ionBreakpointDidChange = await page.spyOnEvent('ionBreakpointDidChange');
        const modal = await page.locator('.modal-sheet');
      ...
      test(title('should emit ionBreakpointDidChange when breakpoint is set to 0'), async ({ page }) => {
        const ionBreakpointDidChange = await page.spyOnEvent('ionBreakpointDidChange');
        const modal = await page.locator('.modal-sheet');
      ...
      test(title('should emit ionBreakpointDidChange when the sheet is swiped to breakpoint 0'), async ({ page }) => {
        const ionBreakpointDidChange = await page.spyOnEvent('ionBreakpointDidChange');
    ...
    test(title('it should reset the breakpoint value on dismiss'), async ({ page }) => {
...
configs().forEach(({ title, config }) => {
  test.describe('sheet modal: clicking the handle', () => {
    ...
    test(title('should advance to the next breakpoint when handleBehavior is cycle'), async ({ page }) => {
      const ionBreakpointDidChange = await page.spyOnEvent('ionBreakpointDidChange');
      const ionModalDidPresent = await page.spyOnEvent('ionModalDidPresent');
      const modal = page.locator('ion-modal');
    ...
    test(title('should not advance the breakpoint when handleBehavior is none'), async ({ page }) => {
      const ionModalDidPresent = await page.spyOnEvent('ionModalDidPresent');
      const modal = page.locator('ion-modal');
    ...
    test(title('should not dismiss the modal when backdrop is clicked and breakpoint is moving'), async ({ page }) => {
      const ionBreakpointDidChange = await page.spyOnEvent('ionBreakpointDidChange');
      const ionModalDidPresent = await page.spyOnEvent('ionModalDidPresent');
      const modal = page.locator('ion-modal');
