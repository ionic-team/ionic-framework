...
configs({ modes: ['ios'], directions: ['ltr'] }).forEach(({ title, config }) => {
  test.describe('datetime: values', () => {
    test(title('should render correct days'), async ({ page }) => {
    ...
    test(title('should render correct months'), async ({ page }) => {
      await page.setContent(
        `
    ...
    test(title('should render correct years'), async ({ page }) => {
      await page.setContent(
        `
    ...
    test(title('should render correct hours'), async ({ page }) => {
      await page.setContent(
        `
    ...
    test(title('should render correct minutes'), async ({ page }) => {
      await page.setContent(
        `
    ...
    test(title('should adjust default parts for allowed hour and minute values'), async ({ page }) => {
      /**
    ...
    test(title('should adjust default parts month for allowed month values'), async ({ page }) => {
      /**
       * Mock today's date for testing.
       * Playwright does not support this natively
       * so we extend the native Date interface: https://github.com/microsoft/playwright/issues/6347
    ...
    test(title('today date highlight should persist even if disallowed from dayValues'), async ({ page }) => {
      /**
       * Mock today's date for testing.
       * Playwright does not support this natively
       * so we extend the native Date interface: https://github.com/microsoft/playwright/issues/6347
...
configs({ modes: ['md'], directions: ['ltr'] }).forEach(({ title, config }) => {
  test(title('setting value to empty string should treat it as having no date'), async ({ page }) => {
    await page.setContent(
      `
