...
configs({
  ...
  ({ title, config, screenshot }) => {
    ...
    `,
        ...
        test('month-year button should be disabled', async ({
          page,
        }) => {
          await page.setContent(
            `
    ...
    `,
          ...
          await expect(
            calendarMonthYear.locator(
              'button'
            )
          ).toBeDisabled();
        });

        test('next and prev buttons should be disabled', async ({
          page,
        }) => {
          await page.setContent(
            `
    ...
    `,
          ...
          const prevMonthButton =
            page.locator(
              'ion-datetime .calendar-next-prev ion-button:first-of-type button'
            );
          const nextMonthButton =
            page.locator(
              'ion-datetime .calendar-next-prev ion-button:last-of-type button'
            );
    ...
    `,
        ...
        test('should not navigate through months via right arrow key', async ({
          page,
        }) => {
          await page.setContent(
            `
