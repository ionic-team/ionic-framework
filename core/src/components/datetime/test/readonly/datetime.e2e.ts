...
configs({
  ...
  ({ title, config, screenshot }) => {
    ...
    `,
        ...
        test('date should be disabled', async ({
          page,
        }) => {
          await page.setContent(
            `
    ...
    `,
          ...
          const febFirstButton =
            page.locator(
              `.calendar-day[data-day='1'][data-month='2']`
            );
        ...
        test('should navigate months via month-year button', async ({
          page,
        }) => {
          await page.setContent(
            `
    ...
    `,
          ...
          await page
            .locator(
              '.month-column .picker-item[data-value="3"]'
            )
            .click();
          await page.waitForChanges();
          await expect(
        ...
        test('should open picker using keyboard navigation', async ({
          page,
          browserName,
        }) => {
          await page.setContent(
    ...
    `,
        ...
        test('should view next month via next button', async ({
          page,
        }) => {
          await page.setContent(
            `
    ...
    `,
        ...
        test('should not change value when the month is changed via keyboard navigation', async ({
          page,
          browserName,
        }) => {
          await page.setContent(
    ...
    `,
          ...
          const prevButton =
            page.locator(
              '.calendar-next-prev ion-button:nth-child(1)'
            );
          const nextButton =
            page.locator(
              '.calendar-next-prev ion-button:nth-child(2)'
            );
          const calendarMonthYear =
            page.locator(
              'ion-datetime .calendar-month-year'
