  ...
  test.describe('datetime: disable dates', () => {
    test.describe('check return values', () => {
      ...
      test.describe('when isDateEnabled returns true', () => {
        test(title('calendar days should be enabled'), async ({ page }) => {
      ...
      test.describe('when isDateEnabled returns false', () => {
        test(title('calendar days should be disabled'), async ({ page }) => {
          const datetime = page.locator('ion-datetime');
    ...
    test.describe('check example usages', () => {
      ...
      test(title('should disable a specific date'), async ({ page }) => {
        const disabledDay = queryAllDisabledDays(page, '#specificDate');

        await expect(disabledDay).toHaveText('10');
      });

      test(title('should disable specific days of the week'), async ({ page }) => {
      ...
      test(title('should disable a range of dates'), async ({ page }) => {
      ...
      test(title('should disable a month'), async ({ page }) => {
    ...
    test.describe('with a min date range', () => {
      test(title('should not enable already disabled dates'), async ({ page }) => {
