...
configs({
  ...
  test.describe(
    title('datetime: minmax'),
    () => {
      test('calendar arrow navigation should respect min/max values', async ({
        page,
      }) => {
        test.info().annotations.push({
          type: 'issue',
          description:
            'https://github.com/ionic-team/ionic-framework/issues/25073',
        });
    ...
    `,
        ...
        const prevButton = page.locator(
          'ion-datetime .calendar-next-prev ion-button:nth-child(1)'
        );
        const nextButton = page.locator(
          'ion-datetime .calendar-next-prev ion-button:nth-child(2)'
        );
        ...
        await page.evaluate(
          'initDatetimeChangeEvent()'
        );

        const monthDidChangeSpy =
      ...
      test('datetime: minmax months disabled', async ({
        ...
        await page.goto(
          '/src/components/datetime/test/minmax',
          config
        );
        const calendarMonths =
          page.locator(
            'ion-datetime#inside .calendar-month'
          );
        ...
        await expect(
          calendarMonths.nth(0)
        ).not.toHaveClass(
          /calendar-month-disabled/
        );
        await expect(
          calendarMonths.nth(1)
        ).not.toHaveClass(
          /calendar-month-disabled/
        );
        await expect(
          calendarMonths.nth(2)
        ).toHaveClass(
          /calendar-month-disabled/
        );
      });
      ...
      test('datetime: minmax navigation disabled', async ({
        ...
        const navButtons = page.locator(
          'ion-datetime#outside .calendar-next-prev ion-button'
        );
      ...
      test('datetime: min including day should not disable month', async ({
        ...
        const calendarMonths =
          page.locator(
            'ion-datetime#min-with-day .calendar-month'
          );
      ...
      test.describe(
        'when the datetime does not have a value',
        () => {
          test('all time values should be available for selection', async ({
            page,
          }) => {
            /**
      ...
      test.describe(
        'setting value outside bounds should show in-bounds month',
        () => {
          const testDisplayedMonth =
            async (
              page: E2EPage,
              content: string,
              expectedString = /June 2021/
            ) => {
              await page.setContent(
                content,
                config
          ...
          test('when min and value are defined', async ({
            page,
          }) => {
            await testDisplayedMonth(
              page,
              `<ion-datetime min="2021-06-01" value="2021-05-01"></ion-datetime>`
            );
          });

          test('when max and value are defined', async ({
            page,
          }) => {
            await testDisplayedMonth(
              page,
              `<ion-datetime max="2021-06-30" value="2021-07-01"></ion-datetime>`
            );
          });

          test('when min, max, and value are defined', async ({
            page,
          }) => {
            await testDisplayedMonth(
              page,
          ...
          test('when max is defined', async ({
            page,
          }) => {
            await testDisplayedMonth(
              page,
              `<ion-datetime max="2012-06-01"></ion-datetime>`,
              /June 2012/
            );
          });
        }
      ...
      // TODO(FW-2165)
      test('should not loop infinitely in webkit', async ({
        page,
        skip,
      }) => {
        test.info().annotations.push({
          type: 'issue',
          description:
            'https://github.com/ionic-team/ionic-framework/issues/25752',
        });
      ...
      test('should not include 12AM when minimum is greater than 12AM', async ({
        page,
      }) => {
        test.info().annotations.push({
          type: 'issue',
          description:
            'https://github.com/ionic-team/ionic-framework/issues/25183',
        });
      ...
      test('should include 12PM when minimum is greater than 12', async ({
        page,
      }) => {
        test.info().annotations.push({
          type: 'issue',
      ...
      test.describe(
        'minmax value adjustment when out of bounds',
        () => {
          test('should reset to min time if out of bounds', async ({
            page,
          }) => {
            await page.setContent(
              `
        ...
        `,
            ...
            const dayButton =
              page.locator(
                'ion-datetime .calendar-day[data-day="10"][data-month="10"][data-year="2022"]'
              );
          ...
          test('should reset to max time if out of bounds', async ({
            page,
          }) => {
            await page.setContent(
              `
        ...
        `,
          ...
          test('should adjust to in-bounds when using month picker', async ({
            page,
          }) => {
            await page.setContent(
              `
        ...
        `,
            ...
            const monthColumnItems =
              page.locator(
                'ion-datetime .month-column .picker-item:not(.picker-item-empty)'
              );
            const ionChange =
              await page.spyOnEvent(
                'ionChange'
            ...
            await monthColumnItems
              .nth(0)
              .click(); // switch to January
            await ionChange.next();
      ...
      test.describe(
        'datetime: confirm button',
        () => {
          test('should apply max and min constraints even when user confirmation is required', async ({
            page,
          }) => {
            test
              .info()
        ...
        `,
            ...
            const maxDate =
              page.locator(
                'ion-datetime .calendar-day[data-day="10"][data-month="1"][data-year="2022"]'
              );
            await maxDate.click();
            await page.waitForChanges();
