...
test.describe('datetime: minmax', () => {
  configs().forEach(({ title, config }) => {
    test(title('calendar arrow navigation should respect min/max values'), async ({ page }) => {
    ...
    test(title('datetime: minmax months disabled'), async ({ page }) => {
    ...
    test(title('datetime: minmax navigation disabled'), async ({ page }) => {
      await page.goto('/src/components/datetime/test/minmax', config);
      await page.waitForSelector('.datetime-ready');
    ...
    test(title('datetime: min including day should not disable month'), async ({ page }) => {
      await page.goto('/src/components/datetime/test/minmax', config);
      await page.waitForSelector('.datetime-ready');
  ...
  configs().forEach(({ title, config }) => {
    test.describe('when the datetime does not have a value', () => {
      test(title('all time values should be available for selection'), async ({ page }) => {
        /**
  ...
  configs({ directions: ['ltr'] }).forEach(({ title, config }) => {
    test.describe('setting value outside bounds should show in-bounds month', () => {
      const testDisplayedMonth = async (
        page: E2EPage,
      ...
      test(title('when min and value are defined'), async ({ page }) => {
        await testDisplayedMonth(page, config, `<ion-datetime min="2021-06-01" value="2021-05-01"></ion-datetime>`);
      });

      test(title('when max and value are defined'), async ({ page }) => {
        await testDisplayedMonth(page, config, `<ion-datetime max="2021-06-30" value="2021-07-01"></ion-datetime>`);
      });

      test(title('when min, max, and value are defined'), async ({ page }) => {
      ...
      test(title('when max is defined'), async ({ page }) => {
        await testDisplayedMonth(page, config, `<ion-datetime max="2012-06-01"></ion-datetime>`, 'June 2012');
      });
    });
  });
  configs().forEach(({ title, config }) => {
    test(title('should not loop infinitely in webkit'), async ({ page, skip }) => {
      test.info().annotations.push({
        type: 'issue',
  ...
  // TODO(FW-2165)
  configs({ directions: ['ltr'] }).forEach(({ title, config }) => {
    test(title('should not include 12AM when minimum is greater than 12AM'), async ({ page }) => {
      test.info().annotations.push({
        type: 'issue',
    ...
    test(title('should include 12PM when minimum is greater than 12'), async ({ page }) => {
      test.info().annotations.push({
        type: 'issue',
        description: 'https://github.com/ionic-team/ionic-framework/issues/25183',
      });
  ...
  configs({ modes: ['md'], directions: ['ltr'] }).forEach(({ title, config }) => {
    test.describe('minmax value adjustment when out of bounds', () => {
      test(title('should reset to min time if out of bounds'), async ({ page }) => {
        await page.setContent(
          `
          <ion-datetime
      ...
      test(title('should reset to max time if out of bounds'), async ({ page }) => {
        await page.setContent(
          `
          <ion-datetime
  ...
  configs({ directions: ['ltr'] }).forEach(({ title, config }) => {
    test.describe('datetime: confirm button', () => {
      test(title('should apply max and min constraints even when user confirmation is required'), async ({ page }) => {
        test.info().annotations.push({
          type: 'issue',
          description: 'https://github.com/ionic-team/ionic-framework/issues/25073',
        });
