...
const MULTIPLE_DATES_SEPARATE_MONTHS = [
  '2022-03-01',
  '2022-04-01',
  '2022-05-01',
];
...
class DatetimeMultipleFixture {
  ...
  async goto(
    ...
    value?: string | string[],
    datetimeConfig?: DatetimeMultipleConfig
  ) {
    const {
      showDefaultButtons,
      showClearButton,
    ...
    this.datetime = this.page.locator(
      'ion-datetime'
    );
    await this.page
  ...
  async expectMultipleDatePicker(
    id: string,
...
configs().forEach(
  ({ title, screenshot, config }) => {
    test.describe(
      title(
        'datetime: multiple date selection (visual regressions)'
      ),
      () => {
        ...
        test('single default value should not have visual regressions', async () => {
          ...
          await datetimeFixture.expectMultipleDatePicker(
            'singleDefaultValue',
            screenshot
          );
        });
...
configs({
  ...
  test.describe(
    title(
      'datetime: multiple date selection (functionality)'
    ),
    () => {
      let datetimeFixture: DatetimeMultipleFixture;
      test.beforeEach(
      ...
      test('clicking unselected days should select them', async ({
        page,
      }) => {
        const datetime =
          await datetimeFixture.goto(
      ...
      test('clicking selected days should unselect them', async ({
        page,
      }) => {
        const datetime =
          await datetimeFixture.goto(
      ...
      test('change event should emit with array detail', async ({
        page,
      }) => {
        const datetime =
          await datetimeFixture.goto(
      ...
      test('should scroll to new month when value is updated with multiple dates in the same month', async ({
        page,
      }) => {
        test.info().annotations.push({
          type: 'issue',
          description:
            'https://github.com/ionic-team/ionic-framework/issues/28602',
        });
        const datetime =
          await datetimeFixture.goto(
            config,
      ...
      test('should not scroll to new month when value is updated with dates in different months', async ({
        page,
      }) => {
        const datetime =
          await datetimeFixture.goto(
      ...
      test('with buttons, should only update value when confirm is called', async ({
        page,
      }) => {
        const datetime =
          await datetimeFixture.goto(
      ...
      test('clicking day when no default value should set value to only clicked day', async ({
        page,
      }) => {
        const datetime =
          await datetimeFixture.goto(
        ...
        const firstDayButton =
          datetime.locator(
            '.calendar-month:nth-child(2) [data-day="1"]'
          );

        const year =
          await firstDayButton.getAttribute(
            'data-year'
          );
        let month =
          await firstDayButton.getAttribute(
      ...
      test('header text should render default date when multiple="false"', async ({
        page,
      }) => {
