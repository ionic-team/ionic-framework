...
configs({ directions: ['ltr'] }).forEach(({ title, config }) => {
  test.describe('datetime: set-value', () => {
    test(title('should update the active date when value is initially set'), async ({ page }) => {
    ...
    test(title('should update the active time when value is initially set'), async ({ page }) => {
      await page.goto('/src/components/datetime/test/set-value', config);
      await page.waitForSelector('.datetime-ready');
    ...
    test(title('should update active item when value is not initially set'), async ({ page }) => {
