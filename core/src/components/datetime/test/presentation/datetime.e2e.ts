...
configs().forEach(
  ({ title, screenshot, config }) => {
    test.describe(
      title(
        'datetime: presentation rendering'
      ),
      () => {
        let presentationFixture!: DatetimePresentationFixture;
        test.beforeEach(({ page }) => {
          presentationFixture =
            new DatetimePresentationFixture(
              page
            );
        });

        test('should not have visual regressions with date-time presentation', async () => {
          await presentationFixture.goto(
            'date-time',
            config
          );
          await presentationFixture.screenshot(
            'datetime-presentation-date-time-diff',
            screenshot
          );
        });
        ...
        test('should not have visual regressions with time-date presentation', async () => {
          ...
          await presentationFixture.screenshot(
            'datetime-presentation-time-date-diff',
            screenshot
          );
        });
        ...
        test('should not have visual regressions with time presentation', async () => {
          ...
          await presentationFixture.screenshot(
            'datetime-presentation-time-diff',
            screenshot
          );
        });
        ...
        test('should not have visual regressions with date presentation', async () => {
          ...
          await presentationFixture.screenshot(
            'datetime-presentation-date-diff',
            screenshot
          );
        });
        ...
        test('should not have visual regressions with month-year presentation', async () => {
          ...
          await presentationFixture.screenshot(
            'datetime-presentation-month-year-diff',
            screenshot
          );
        });
        ...
        test('should not have visual regressions with month presentation', async () => {
          ...
          await presentationFixture.screenshot(
            'datetime-presentation-month-diff',
            screenshot
          );
        });
        ...
        test('should not have visual regressions with year presentation', async () => {
          ...
          await presentationFixture.screenshot(
            'datetime-presentation-year-diff',
            screenshot
          );
        });
      }
...
configs({
  ...
  test.describe(
    title('datetime: presentation'),
    () => {
      test('presentation: time: should emit ionChange', async ({
        page,
      }) => {
        await page.goto(
          `/src/components/datetime/test/presentation`,
          config
        );
      ...
      test('presentation: month-year: should emit ionChange', async ({
        page,
      }) => {
        await page.goto(
          `/src/components/datetime/test/presentation`,
      ...
      test('presentation: month: should emit ionChange', async ({
        page,
      }) => {
        await page.goto(
          `/src/components/datetime/test/presentation`,
      ...
      test('presentation: year: should emit ionChange', async ({
        page,
      }) => {
        await page.goto(
          `/src/components/datetime/test/presentation`,
      ...
      test('switching presentation should close month/year picker', async ({
        page,
      }) => {
  ...
  test.describe(
    ...
    () => {
      test('changing value from AM to AM should update the text', async ({
        ...
        const timePickerFixture =
          new TimePickerFixture(page);
        await timePickerFixture.goto(
          '04:20:00',
          config
        );

        await timePickerFixture.setValue(
          '11:03:00'
        );
        await timePickerFixture.expectTime(
          11,
          3,
          'am'
        );
...
class TimePickerFixture {
  ...
  async goto(
    ...
    await this.page
      .locator('.datetime-ready')
      .waitFor();
    this.timePicker = this.page.locator(
      'ion-datetime'
    );
  }

  async setValue(value: string) {
    await this.timePicker.evaluate(
      (
        el: HTMLIonDatetimeElement,
        newValue: string
      ) => {
  ...
  async expectTime(
    ...
    const pickerColumns =
      this.timePicker.locator(
        'ion-picker-column-internal'
      );

    await expect(
      pickerColumns.nth(0)
    ).toHaveJSProperty('value', hour);
    await expect(
      pickerColumns.nth(1)
    ).toHaveJSProperty('value', minute);
    await expect(
      pickerColumns.nth(2)
    ).toHaveJSProperty('value', ampm);
  }
}
