    ...
    test.describe(
      ...
      () => {
        ...
        test.describe('ja-JP', () => {
          ...
          test('should correctly localize calendar day buttons without literal', async ({
            page,
          }) => {
            await datetimeFixture.goto(
              config,
            ...
            const datetimeButtons =
              page.locator(
                'ion-datetime .calendar-day:not([disabled])'
              );
             ...
             */
            await expect(
              datetimeButtons.nth(0)
            ).toHaveText('1');
            await expect(
              datetimeButtons.nth(1)
            ).toHaveText('2');
            await expect(
              datetimeButtons.nth(2)
            ).toHaveText('3');
          });
        });
...
configs({
  ...
  test.describe(title('ar-EG'), () => {
    test('should correctly localize calendar day buttons', async ({
      page,
    }) => {
    ...
    test('should correctly localize year column data', async ({
      page,
    }) => {
      await page.setContent(
        `
      ...
      const datetimeYear = page.locator(
        'ion-datetime .year-column .picker-item[data-value="2022"]'
      );
