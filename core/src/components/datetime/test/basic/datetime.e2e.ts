...
configs().forEach(({ title, config }) => {
  test.describe('datetime: closing time popover', () => {
    test(title('it should not change months'), async ({ page }) => {
...
configs().forEach(({ title, config }) => {
  test.describe('datetime: selecting a day', () => {
    ...
    test(title('should not highlight a day until one is selected'), async ({ page }) => {
      await testHighlight(page, 'inline-datetime-no-value');
    });

    test(title('should not highlight a day until one is selected, with default-buttons'), async ({ page }) => {
...
configs().forEach(({ title, config }) => {
  test.describe('datetime: confirm date', () => {
    test(title('should not update value if Done was clicked without selecting a day first'), async ({ page }) => {
      await page.goto('/src/components/datetime/test/basic', config);
...
configs().forEach(({ title, config }) => {
  test.describe('datetime: footer', () => {
    test(title('should render default buttons'), async ({ page }) => {
    ...
    test(title('should render clear button'), async ({ page }) => {
    ...
    test(title('should render default and clear buttons'), async ({ page }) => {
      await page.setContent(
    ...
    test(title('should render custom buttons'), async ({ page }) => {
      await page.setContent(
        `
...
configs({ modes: ['md'], directions: ['ltr'] }).forEach(({ title, config }) => {
  test.describe('datetime: swiping', () => {
    test(title('should move to prev month by swiping'), async ({ page }) => {
      await page.setContent(
        `
    ...
    test(title('should move to next month by swiping'), async ({ page }) => {
      await page.setContent(
        `
        <ion-datetime value="2022-05-03"></ion-datetime>
      `,
    ...
    test(title('should not re-render if swipe is in progress'), async ({ page, skip }) => {
...
configs({ modes: ['ios'], directions: ['ltr'] }).forEach(({ title, config }) => {
  test.describe('datetime: visibility', () => {
    test(title('should reset month/year interface when hiding datetime'), async ({ page }) => {
      await page.setContent(
        `
        <ion-datetime></ion-datetime>
      `,
...
configs({ modes: ['ios'], directions: ['ltr'] }).forEach(({ title, config }) => {
  test.describe('datetime: RTL set on component', () => {
    test(title('should flip icons when RTL is set on component directly'), async ({ page }) => {
      await page.setContent(
        `
...
configs({ modes: ['ios'], directions: ['ltr'] }).forEach(({ title, config }) => {
  test.describe('datetime: clear button', () => {
    test(title('should clear the active calendar day'), async ({ page }, testInfo) => {
