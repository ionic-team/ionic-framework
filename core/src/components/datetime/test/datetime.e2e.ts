...
configs({
  ...
  test.describe(
    title(
      'datetime: switching months with different number of days'
    ),
    () => {
      test.beforeEach(
        async ({ page }) => {
      ...
      test('should adjust the selected day when moving to a month with a different number of days', async ({
        page,
      }) => {
        const monthYearToggle =
          page.locator(
