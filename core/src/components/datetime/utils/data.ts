...
export const generateTime = (
  ...
  const use24Hour = is24Hour(
    computedHourCycle
  );
  let processedHours = getHourData(
    computedHourCycle
  );
...
export const getMonthColumnData = (
  ...
  if (monthValues !== undefined) {
    ...
    processedMonths.forEach(
      (processedMonth) => {
        const date = new Date(
          `${processedMonth}/1/${year} GMT+0000`
        );
    ...
    const maxMonth =
      maxParts && maxParts.year === year
        ? maxParts.month
        : 12;
    const minMonth =
      minParts && minParts.year === year
        ? minParts.month
        : 1;
...
export const getCombinedDateColumnData =
  (
     ...
     * single year.
     */
    let months = getAllMonthsInRange(
      minParts,
      maxParts
    );
...
export const getTimeColumnsData = (
  ...
  const minutesItems = minutes.map(
    (minute) => {
      return {
