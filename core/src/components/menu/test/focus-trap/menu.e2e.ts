...
configs({
  ...
  test.describe(
    title('menu: focus trap'),
    () => {
      test.beforeEach(
        async ({ page, skip }) => {
          await page.goto(
            `/src/components/menu/test/focus-trap`,
            config
          );
      ...
      test('should trap focus with overlays', async ({
        page,
        browserName,
      }) => {
        const ionDidOpen =
      ...
      test('should trap focus with content inside overlays', async ({
        page,
      }) => {
        const ionDidOpen =
          await page.spyOnEvent(
      ...
      test('should work with swipe gestures after modal is dismissed', async ({
        page,
      }) => {
        const ionDidOpen =
          await page.spyOnEvent(
