  ...
  test.describe(
    ...
    () => {
      ...
      test('should preserve scroll position', async ({
        ...
        skip.browser(
          'firefox',
          'Firefox does not preserve scroll position'
        );
        ...
        await page
          .locator(
            '#start-menu ion-content'
          )
          .evaluate(
            async (
              el: HTMLIonContentElement
            ) => {
              await el.scrollToPoint(
                0,
                200
              );
            }
        ...
        const scrollTop = await page
          ...
          .evaluate(
            async (
              ...
              const contentScrollEl =
                await el.getScrollElement();
              return contentScrollEl.scrollTop;
            }
          );

        await expect(scrollTop).toBe(
          200
        );
      });
    }
  ...
  ({ title, screenshot, config }) => {
    test.describe(
      ...
      () => {
        ...
        test('should render on the correct side when side is changed dynamically', async ({
          page,
        }) => {
          test.info().annotations.push({
            type: 'issue',
            description:
              'https://github.com/ionic-team/ionic-framework/issues/25601',
          });
        ...
        test('should render on the correct side when document direction is changed dynamically', async ({
          page,
        }) => {
          test.info().annotations.push({
            type: 'issue',
