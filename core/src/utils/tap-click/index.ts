...
export const startTapClick = (
  ...
  const setActivatedElement = (
    ...
    if (activatableEle) {
      if (
        clearDefers.has(activatableEle)
      ) {
        throw new Error(
          'internal error'
        );
      ...
      if (
        !activatableEle.classList.contains(
          ACTIVATED
        )
      ) {
        addActivated(
