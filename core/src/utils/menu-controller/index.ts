...
import type {
  MenuI,
  MenuControllerI,
} from '../../components/menu/menu-interface';
  ...
  (): MenuControllerI => {
    ...
    const _setOpen = async (
      ...
      if (shouldOpen) {
        ...
        if (
          ...
          await openedMenu.setOpen(
            false,
            false
          );
        }
      }
      return menu._setOpen(
        shouldOpen,
        animated
      );
    };
    ...
    const _createAnimation = (
      ...
      const animationBuilder =
        menuAnimations.get(type) as any; // TODO(FW-2832): type
      if (!animationBuilder) {
        throw new Error(
