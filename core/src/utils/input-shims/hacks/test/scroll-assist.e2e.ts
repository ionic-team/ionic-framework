...
const getScrollPosition = async (
  ...
  return await contentEl.evaluate(
    async (
...
configs({
  ...
  test.describe.skip(
    ...
    () => {
      ...
      test.describe(
        'scroll-assist: basic functionality',
        () => {
      ...
      test.describe(
        'scroll-assist: scroll-padding',
        () => {
          test.describe(
            'scroll-padding: browser/cordova',
            () => {
              test.beforeEach(
                async () => {
                  await scrollAssistFixture.goto(
          ...
          test.describe(
            'scroll-padding: webview resizing',
            () => {
...
class ScrollAssistFixture {
  ...
  async goto(
    ...
    this.content = this.page.locator(
      'ion-content'
    );
  }
  ...
  async expectNotToHaveScrollAssist(
    interactiveSelector: string,
    inputSelector: string
  ) {
    await expect(
  ...
  async expectToHaveScrollAssist(
    interactiveSelector: string,
    inputSelector: string
  ) {
    await expect(
  ...
  async expectToHaveScrollPadding(
    interactiveSelector: string,
    inputSelector: string
  ) {
    const { content } = this;
    ...
    await expect(content).not.toHaveCSS(
      '--keyboard-offset',
      '0px'
    );
  }

  async expectNotToHaveScrollPadding(
    interactiveSelector: string,
    inputSelector: string
  ) {
    const { content } = this;
