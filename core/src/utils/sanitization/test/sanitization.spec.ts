...
describe('sanitizeDOMString', () => {
  ...
  it('filter onclick', () => {
    expect(
      ...
      '<button id="myButton" name="myButton">harmless button</button>'
    );
  });
  ...
  it('filter <a> href JS', () => {
    ...
    expect(
      sanitizeDOMString(
        '<a href="javascr&Tab;ipt:alert(document.cookie)">harmless link</a>'
      )
    ).toEqual('<a>harmless link</a>');
  });
  ...
  it('filter href + javascript ', () => {
    expect(
      ...
      '<div><button><a>click me</a></button></div>'
    );
  });
...
const enableSanitizer = (
  ...
  (
    window as any
  ).Ionic.config.sanitizerEnabled = enable;
};
