...
export const detachComponent = (
  ...
  if (element) {
    if (delegate) {
      ...
      return delegate.removeViewFromDom(
        container,
        element
      );
    }
  ...
  const attachViewToDom = async (
    ...
    if (userComponent) {
      ...
      const el: any =
        ...
        'string'
          ? BaseComponent.ownerDocument?.createElement(
              userComponent
            )
          : userComponent;
     ...
     * it was previously.
     */
    Reference = document.createComment(
      'ionic teleport'
    );
    BaseComponent.parentNode.insertBefore(
      Reference,
      BaseComponent
    );
