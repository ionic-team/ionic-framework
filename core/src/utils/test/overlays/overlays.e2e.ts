...
configs({ directions: ['ltr'] }).forEach(({ title, config }) => {
  test.describe('overlays: dismiss', () => {
    ...
    test(title('hardware back button: should dismiss a presented overlay'), async ({ page }) => {
    ...
    test(
      title(
        'hardware back button: should dismiss the presented overlay, even though another hidden modal was added last'
      ),
      async ({ page }) => {
        const ionModalDidPresent = await page.spyOnEvent('ionModalDidPresent');
        const ionModalDidDismiss = await page.spyOnEvent('ionModalDidDismiss');
    ...
    test(title('Esc: should dismiss a presented overlay'), async ({ page }) => {
    ...
    test(
      title('Esc: should dismiss the presented overlay, even though another hidden modal was added last'),
      async ({ page }) => {
        const ionModalDidPresent = await page.spyOnEvent('ionModalDidPresent');
        const ionModalDidDismiss = await page.spyOnEvent('ionModalDidDismiss');
    ...
    test(title('overlays: Nested: should dismiss the top overlay'), async ({ page }) => {
      const ionModalDidPresent = await page.spyOnEvent('ionModalDidPresent');
      const ionModalDidDismiss = await page.spyOnEvent('ionModalDidDismiss');
...
configs({ directions: ['ltr'] }).forEach(({ title, config }) => {
  test.describe('overlays: focus', () => {
    test(title('should not focus the overlay container if element inside of overlay is focused'), async ({ page }) => {
    ...
    test(title('should not select a hidden focusable element'), async ({ page, browserName }) => {
      await page.setContent(
        `
        <style>
          [hidden] {
    ...
    test(title('should not select a disabled focusable element'), async ({ page, browserName }) => {
      await page.setContent(
        `
        <ion-button id="open-modal">Show Modal</ion-button>
        <ion-modal trigger="open-modal">
    ...
    test(title('should select a focusable element with disabled="false"'), async ({ page, browserName }) => {
      await page.setContent(
        `
        <ion-button id="open-modal">Show Modal</ion-button>
        <ion-modal trigger="open-modal">
    ...
    test(title('toast should not cause focus trapping'), async ({ page }) => {
      await page.goto('/src/utils/test/overlays', config);
    ...
    test(
      title('toast should not cause focus trapping even when opened from a focus trapping overlay'),
      async ({ page }) => {
        await page.goto('/src/utils/test/overlays', config);
    ...
    test(title('focus trapping should only run on the top-most overlay'), async ({ page }) => {
      await page.goto('/src/utils/test/overlays', config);
