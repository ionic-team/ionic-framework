...
describe('Platform Tests', () => {
  describe('testUserAgent()', () => {
    it('should return true when testing if user agent is an iPhone', () => {
      const win = configureBrowser(
        PlatformConfiguration.iPhone
      );
      expect(
        testUserAgent(win, /iPhone/)
      ).toEqual(true);
    });
    ...
    it('should return true when testing if user agent is an Android', () => {
      const win = configureBrowser(
        PlatformConfiguration.AndroidTablet
      );
      expect(
        testUserAgent(
          win,
  ...
  describe('getPlatforms()', () => {
    it('should contain "desktop" platform', () => {
      const win = configureBrowser(
        PlatformConfiguration.DesktopSafari
      );
      expect(
        getPlatforms(win)
      ).toContain('desktop');
    ...
    it('should contain "capacitor" platform', () => {
      const win = configureBrowser(
        PlatformConfiguration.Capacitor
      );
      expect(
        getPlatforms(win)
      ).toContain('capacitor');
  ...
  describe('isPlatform()', () => {
    it('should return true for "capacitor" and "hybrid" in a Capacitor app', () => {
      ...
      expect(
        isPlatform(win, 'capacitor')
      ).toEqual(true);
      expect(
        isPlatform(win, 'hybrid')
      ).toEqual(true);
    });
    ...
    it('should return true for "android" and "tablet" and false for "ios" on an android tablet', () => {
      ...
      expect(
        isPlatform(win, 'android')
      ).toEqual(true);
      expect(
        isPlatform(win, 'tablet')
      ).toEqual(true);
      expect(
        isPlatform(win, 'ios')
      ).toEqual(false);
    ...
    it('should return true for "cordova" and "hybrid" in a Cordova app', () => {
      const win = configureBrowser(
        PlatformConfiguration.Cordova
      );
      expect(
        isPlatform(win, 'cordova')
      ).toEqual(true);
      expect(
        isPlatform(win, 'hybrid')
      ).toEqual(true);
    ...
    it('should return true for "android", false for "tablet, and false for "desktop"" on a Pixel 2 XL', () => {
      const win = configureBrowser(
        PlatformConfiguration.Pixel2XL
      );
      expect(
        isPlatform(win, 'android')
      ).toEqual(true);
    ...
    it('should return true for "android" and "tablet" and false for "desktop" on a Galaxy View', () => {
      const win = configureBrowser(
        PlatformConfiguration.GalaxyView
      );
      expect(
        isPlatform(win, 'android')
      ).toEqual(true);
    ...
    it('should return true for "android", false for "tablet", and false for "desktop" on Galaxy S9 Plus', () => {
      const win = configureBrowser(
        PlatformConfiguration.GalaxyS9Plus
      );
      expect(
        isPlatform(win, 'android')
      ).toEqual(true);
