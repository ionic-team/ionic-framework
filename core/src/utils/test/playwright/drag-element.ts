...
export const dragElementBy = async (
  ...
  const startX =
    startXCoord === undefined
      ? boundingBox.x +
        boundingBox.width / 2
      : startXCoord;
  const startY =
    startYCoord === undefined
      ? boundingBox.y +
 ...
 */
export const dragElementByYAxis =
  async (
    el:
      | Locator
      | ElementHandle<
...
const validateDragByX = (
  ...
  if (endX > viewportWidth) {
    const recommendedDragByX =
      viewportWidth - startX - 5;
    throw new Error(
...
const validateDragByY = (
  ...
  if (endY > viewportHeight) {
    const recommendedDragByY =
      viewportHeight - startY - 5;
    throw new Error(
