 ...
 *
 */
export const setIonViewport = async (
  page: Page,
  options?: SetIonViewportOptions
) => {
  const currentViewport =
    page.viewportSize();
  const ionContent = await page.$(
    'ion-content'
  );

  if (ionContent) {
    await ionContent.waitForElementState(
      'stable'
    );
  }
  ...
  const [x, y] = await page.evaluate(
    async () => {
      ...
      if (content) {
        const innerScroll =
          content.shadowRoot!.querySelector(
            '.inner-scroll'
          )!;
        return [
