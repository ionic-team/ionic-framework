...
const detectPlatforms = (
  ...
  return (
    ...
    const customMethod =
      customPlatformMethods?.[p];
    return typeof customMethod ===
      'function'
      ? customMethod(win)
      : PLATFORMS_MAP[p](win);
  });
...
const isIOS = (win: Window) =>
  testUserAgent(win, /iPhone|iPod/i) ||
  isIpad(win);
...
export type PlatformConfig = Partial<
  typeof PLATFORMS_MAP
>;
