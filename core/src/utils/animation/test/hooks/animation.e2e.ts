...
configs({
  ...
  test.describe(
    ...
    async () => {
      test(`should fire hooks using web animations`, async ({
        page,
      }) => {
        await page.goto(
          '/src/utils/animation/test/hooks',
          config
        );
        await testHooks(page);
      });

      test(`should fire hooks using css animations`, async ({
        page,
      }) => {
        await page.goto(
          '/src/utils/animation/test/hooks?ionic:_forceCSSAnimations=true',
          config
        );
        await testHooks(page);
      });
...
const testHooks = async (
  ...
  const beforeRead =
    await page.spyOnEvent('beforeRead');
  const beforeWrite =
    await page.spyOnEvent(
      'beforeWrite'
    );
  const afterRead =
    await page.spyOnEvent('afterRead');
  const afterWrite =
    await page.spyOnEvent('afterWrite');

  // Test initial classes
  await expect(square).toHaveClass(
    /hello-world/
  );
  await expect(square).not.toHaveClass(
    /test-class/
  );

  // Test initial styles
  await expect(square).toHaveCSS(
    'padding-bottom',
    '20px'
  );
  await expect(square).toHaveCSS(
