 ...
 */
type AnimationReadWriteCallback =
  () => void;
...
export const createAnimation = (
  ...
  let finished = false;
  let shouldCalculateNumAnimations =
    true;
  ...
  const supportsWebAnimations =
    typeof (Element as any) ===
      'function' &&
    typeof (Element as any).prototype!
      .animate === 'function' &&
  ...
  const clearCallback = (
    ...
    callbackObjects:
      | AnimationOnFinishCallback[]
      | AnimationOnStopCallback[]
  ) => {
    const index =
      callbackObjects.findIndex(
        (callbackObject) =>
          callbackObject.c ===
          callbackToRemove
      );
  ...
  const beforeAddRead = (
    readFn: AnimationReadWriteCallback
  ) => {
    _beforeAddReadFunctions.push(
      readFn
    );
  ...
  const beforeAddWrite = (
    writeFn: AnimationReadWriteCallback
  ) => {
    _beforeAddWriteFunctions.push(
      writeFn
    );
  ...
  const addElement = (
    ...
    if (el != null) {
      if ((el as Node).nodeType === 1) {
        ...
        (el as NodeList).length >= 0
      ) {
        for (
          let i = 0;
          i < (el as NodeList).length;
          i++
        ) {
  ...
  const addAnimation = (
    ...
    if (
      (animationToAdd as any) != null
    ) {
      if (
        Array.isArray(animationToAdd)
      ) {
  ...
  const keyframes = (
    keyframeValues: AnimationKeyFrames
  ) => {
    const different =
  ...
  const beforeAnimation = () => {
    ...
    _beforeAddReadFunctions.forEach(
      (callback) => callback()
    );

    // Runs all before write callbacks
    _beforeAddWriteFunctions.forEach(
      (callback) => callback()
    );
  ...
  const afterAnimation = () => {
    ...
    _afterAddReadFunctions.forEach(
      (callback) => callback()
    );

    // Runs all after write callbacks
    _afterAddWriteFunctions.forEach(
      (callback) => callback()
    );
    ...
    onFinishOneTimeCallbacks.forEach(
      (onFinishCallback) => {
        return onFinishCallback.c(
          currentStep,
          ani
  ...
  const initializeCSSAnimation = (
    ...
    const processedKeyframes =
      processKeyframes(_keyframes);
    elements.forEach((element) => {
      if (
        processedKeyframes.length > 0
      ) {
        const keyframeRules =
          generateKeyframeRules(
            processedKeyframes
  ...
  const stop = () => {
    ...
    onStopOneTimeCallbacks.forEach(
      (onStopCallback) =>
  ...
  const from = (
    ...
    const firstFrame = _keyframes[0] as
      | AnimationKeyFrameEdge
      | undefined;
  ...
  const fromTo = (
    ...
    return from(property, fromValue).to(
      property,
      toValue
    );
  };
