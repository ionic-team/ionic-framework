describe('$ionSlideBox controller', function() {
  ...
  it('#add()', inject(function($rootScope, $timeout) {
    ...
    expect(ctrl.shown()).toBe(-1);
    ctrl.add(slide1);
    expect(ctrl.shown()).toBe(0);
    expect(ctrl.count()).toBe(1);
    $timeout.flush();
    expect(slide1.setState).toHaveBeenCalledWith('shown');
    ...
    $timeout.flush();
    expect(slide2.setState).toHaveBeenCalledWith('right');
  }));
  ...
  it('#left()', function() {
    ...
    ctrl.loop(false);
    ctrl.select(0);
    expect(ctrl.left()).toBe(-1);
    ctrl.select(1);
    expect(ctrl.left()).toBe(0);

    ctrl.loop(true);
    ctrl.select(0);
    expect(ctrl.left()).toBe(2);
    ctrl.select(1);
    expect(ctrl.left()).toBe(0);
  });
  ...
  it('#right()', function() {
    ...
    ctrl.loop(false);
    ctrl.select(2);
    expect(ctrl.right()).toBe(-1);
    ctrl.select(1);
    expect(ctrl.right()).toBe(2);

    ctrl.loop(true);
    ctrl.select(2);
    expect(ctrl.right()).toBe(0);
    ctrl.select(1);
    expect(ctrl.right()).toBe(2);
  });
