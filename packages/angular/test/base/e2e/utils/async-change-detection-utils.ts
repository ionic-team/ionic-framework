...
export const expectAsyncUpdateToRender = async (page: Page) => {
  ...
  // Every app uses zone change detection exactly when Zone.js is loaded. A mismatch means
  // the app lost `provideZoneChangeDetection()` and would silently stop covering #31406.
  ...
  if (changeDetection === 'zone') {
    ...
    // Twice the page's settle delay, so a working tick would already have landed.
