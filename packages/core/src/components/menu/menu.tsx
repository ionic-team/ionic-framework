...
export class Menu {
   ...
   * @hidden
   */
  private prepareAnimation(width: number): Promise<void> {
  ...
  setOpen(shouldOpen: boolean, animated: boolean = true): Promise<boolean> {
    ...
    this._before();
    return this._startAnimation(shouldOpen, animated).then(() => {
  ...
  _swipeWillStart(): Promise<void> {
    this._before();
    const menuWidth = this._menuInnerEle.offsetWidth;
    return this.prepareAnimation(menuWidth);
  }
